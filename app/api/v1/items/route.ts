...
export function GET(request: Request) {
  ...
  if (type && !contentTypes.has(type)) {
    return Response.json({ error: "Geçersiz type değeri." }, { status: 400 });
  }
  if (status && !contentStatuses.has(status)) {
    return Response.json({ error: "Geçersiz status değeri." }, { status: 400 });
  }
  ...
  return Response.json(
    {
      ...
      city: data.city,
      count: items.length,
      filters: { type, status, district, query, activeOnly, limit },
      items,
    },
    {
      headers: {
